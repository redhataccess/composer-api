...
namespace bookin\Composer\Console\Formatter;
...
class BootstrapOutputFormatter extends OutputFormatter {
  ...
  public function format($message) {
    ...
    return preg_replace_callback("{\033\[([0-9;]+)m(.*?)\033\[(?:" . $clearEscapeCodes . ";)*?" . $clearEscapeCodes . "m}s", [$this, 'formatHtml'], $formatted);
  }
  ...
  private function formatHtml($matches) {
    ...
    $html = '<' . $tag . ' ' . ($attributes ?? '') . '>' . $matches[2] . '</' . $tag . '>';
    return $html;
